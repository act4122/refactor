...
public class Rules {
	...
	private List<Integer> leftWallPieces = new ArrayList<Integer>(); // Positions of the left
	// wall spaces.
	private List<Integer> rightWallPieces = new ArrayList<Integer>(); // Positions of the right
	// wall spaces.
	...
	public boolean validateMove( Move move ) {
		...
		try {
			...
			List<Integer> tempVec = new ArrayList<Integer>();
			List<Integer> startVec = new ArrayList<Integer>();
			//Vector possibleJumps = new Vector();
			List<Integer> possibleJumps = checkForPossibleJumps( start, pieceType,
					player );
			...
			for ( int count = 1; count < 64; count++ ) {
				if ( theBoard.occupied( count ) ) {
					if ( theBoard.getPieceAt( count ).getColor() == player.getColor() ) {
						...
						if ( !tempVec.isEmpty() ) {
							startVec.add( new Integer( count ) );
			...
			for ( ; i < loop; i++ ) {
				if ( !leftWallPieces.contains( new Integer( piecePosition +
					...
					if ( adjacentSpace && 
						...
						if ( !endSpace ) {
							possibleJumps.add( new Integer( piecePosition
									+ secondSpots[i] ) );
	...
	private void fillWallVector() {
		...
		rightWallPieces.add( new Integer( 7 ) );
		leftWallPieces.add( new Integer( 8 ) );
		rightWallPieces.add( new Integer( 23 ) );
		leftWallPieces.add( new Integer( 24 ) );
		rightWallPieces.add( new Integer( 39 ) );
		leftWallPieces.add( new Integer( 40 ) );
		rightWallPieces.add( new Integer( 55 ) );
		leftWallPieces.add( new Integer( 56 ) );
				...
				if ( leftWallPieces.contains( new Integer( piecePosition ) ) ) {
					if ( jump ) {
						...
						if ( adjacentSpace && ( aPiece.getColor() !=
							...
							// jump that must be made.
							if ( !endSpace ) {
								moves.add( new Integer( piecePosition
										- 14 ) );
							}
						}
					...
					// Otherwise check for a regular move, not a jump.
						...
						adjacentSpace = theBoard.occupied( piecePosition - 7 );
						if ( !adjacentSpace ) {
							moves.add( new Integer( piecePosition - 7 ) );
						}
						adjacentSpace = theBoard.occupied( piecePosition + 9 );
						if ( !adjacentSpace ) {
							moves.add( new Integer( piecePosition + 9 ) );
						}
					}
				}
